...
// AnchorRandomBoxItem 主播随机盒子物品表。
// level 为 SQL 保留字，column tag 直接写列名，GORM 会自动加反引号。
// probability 在 SQL 中为 double(11,4)，Java 中声明为 Integer，
// 此处按 Java 类型规则映射为 int32（如需浮点精度可在 DAO 层转换）。
type AnchorRandomBoxItem struct {
	// ID 主键
...
// TableName 显式指定表名
func (AnchorRandomBoxItem) TableName() string {
	return "anchor_random_box_item"
}
