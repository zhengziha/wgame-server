...
// 调用者负责发送消息给客户端和其他玩家
func PlayerEnterMap(chara *Chara, gameMap *GameMap) {
	...
	for _, otherGid := range appearList {
		otherChara := CharaManagerInstance().GetCharaByGid(otherGid)
		if otherChara != nil {
			log.Info("[game] appear: %s sees %s", otherChara.Name, chara.Name)
		}
	}
}

// PlayerLeaveMap 处理玩家离开地图的核心逻辑
func PlayerLeaveMap(chara *Chara, gameMap *GameMap) {
	if chara == nil || gameMap == nil {
		return
	}
